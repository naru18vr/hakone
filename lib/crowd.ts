...
export const crowdDetails = (spot: Spot): { facility: CrowdInfo; parking: CrowdInfo; road: CrowdInfo; hourly: CrowdInfo[] } => {
  const base = spot.crowdLevel as CrowdInfo["level"];
  ...
  const largeParking = Boolean(spot.parkingSpaces?.match(/(?:8\d|9\d|[1-9]\d{2,})台/));
  const parkingAdjustment = !spot.parkingAvailable ? 1 : largeParking ? -1 : 0;
  const parking: CrowdInfo = { ...common, level: Math.max(1, Math.min(5, base + parkingAdjustment)) as CrowdInfo["level"], reasons: [...common.reasons, "駐車場規模を加味"] };
