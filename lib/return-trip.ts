...
export const defaultReturnSettings: ReturnSettings = { dinnerTime: "18:30", arrivalStation: "東京駅", rentalReturnMinutes: 30, transferMinutes: 20, delayBufferMinutes: 15, returnTrainMinutes: 40 };

const trainMinutes: Record<ReturnSettings["arrivalStation"], number> = { "東京駅": 40, "品川駅": 30, "新宿駅": 65, "渋谷駅": 60 };
export const getTrainEstimate = (station: ReturnSettings["arrivalStation"], customMinutes?: number) => typeof customMinutes === "number" && Number.isFinite(customMinutes) && customMinutes >= 1 && customMinutes <= 300 ? Math.round(customMinutes) : trainMinutes[station];
...
export const calculateReturnTrip = (day2: ItineraryItem[], spots: Spot[], settings: ReturnSettings = defaultReturnSettings, day2StartTime = "09:00") => {
  ...
  const build = (label: ReturnCase["label"], extraDrive: number): ReturnCase => {
    ...
    const tokyo = departure + getTrainEstimate(settings.arrivalStation, settings.returnTrainMinutes) + settings.delayBufferMinutes;
  ...
  const trainEstimate = getTrainEstimate(settings.arrivalStation, settings.returnTrainMinutes);
  const requiredAfterStation = settings.rentalReturnMinutes + settings.transferMinutes + trainEstimate + settings.delayBufferMinutes;
  return { cases, recommendedStationArrival: [0, 20, 45].map((extra) => formatClock(dinner - requiredAfterStation - extra)), trainEstimate: `${trainEstimate}分（利用者設定）`, summary };
};
